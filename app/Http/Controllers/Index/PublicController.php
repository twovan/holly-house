...
class PublicController extends Controller
{
    public function store($id,Request $request)
    {
        ...
        //修改过期订单状态
        //$dtVancouver->diffInHours($dtOttawa, false)
        //Order::where(['pay_at' => $user['id'] ,'status'=>1])->update(['finished_at' => Carbon::now(), 'status'=> 4]);
        // $fixorderlist=Order::where('status','=',1)
        //                             ->where(Carbon::now() -> diffInHours('pay_at') ,'>=',12 );
                                    //->update(['finished_at' => Carbon::now(), 'status'=> 4])
        $fixorderlist= Order::whereRaw('TIMESTAMPDIFF(HOUR,pay_at,NOW()) >=12 AND `status`=1;')
                          ->update(['finished_at' => Carbon::now(), 'status'=> 4]);
